...
router.get('/get-shoes', async (req, res) => {
    try {
        ...
        res.status(200).render('ShoesView', { shoes: shoes })
...
router.post('/add-shoes', Upload.array('image', 5), async (req, res) => {
    try {
        const { id, idTrademark, name, quantity, price, describe, classify } = req.body
        const { files } = req
...
router.put('/update-shoes', Upload.array('image', 5), async (req, res) => {
    try {
        const { id, idTrademark, name, images, quantity, price, describe, classify, status } = req.body
        const { files } = req
...
router.get('/get-shoes-by-id', async (req, res) => {
    try {
        ...
        if (shoes.length > 0) {
            res.status(201).send({'message': 'Shoes by id', 'data': shoes})
        } else {
...
router.post('/find-shoes-by-id-trademark', async (req, res) => {
    try {
        const idTrademark = req.body.name;
